...
| 8 | Health check extension | `healthcheckextension` README | https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/extension/healthcheckextension | 2026-05-12 | Provides HTTP `/` returning 200 if all pipelines healthy. Default endpoint `:13133`. We expose this for **external** probes (`curl http://localhost:13133/`) rather than the compose container-level healthcheck, because the contrib image is distroless and lacks `wget`/`curl` to probe itself (see D8). |
