...
todos:
  ...
  - id: p5
    ...
    status: completed
  - id: p6
    content: 'Phase 6: services/vendor-service module — self-serve onboarding wizard + KYC (GSTIN / PAN / bank) + admin approval gate before activation (vendor cannot list products until activated) + multi-warehouse registry (geocoded address + state + city + pincode + lat/lng + operating hours + pickup SLA + carrier cut-off times + service-zone polygon-or-pincode-list + enable/disable + ownerType: vendor|platform forward-compat) + vendor subscription tiers + sliding commission schedule + payouts ledger + per-warehouse SLA scoring. SUB-PLAN DRAFTED via .cursor/plans/p6_vendor_service_pr-16_73c63961.plan.md (awaiting user review). Scope decisions baked: single PR-16 covering @repo/types india.ts + @repo/validators/vendor (8 files populating the P2 empty shell, incl. inline GSTIN mod-36 checksum) + @repo/events/vendor (5 v1 events populating the empty shell) + services/vendor-service end-to-end (6 Mongoose schemas + 7 services + 6 controllers + RequireRole decorator + inline 6-step linear onboarding state-machine + Nominatim geocoder with 1 req/sec semaphore + 24h LRU cache + dual-mode service-zone discriminated union pincodes-or-2dsphere-polygon + Better-Auth vendor-org binding) + apps/api-gateway wiring. Payouts + per-warehouse SLA scoring SCHEMAS ship; writer cron deferred to P10/P21 per D14/D15. 5 open questions parked for user input pre-execution (Q1 geocoding provider OSM vs MapMyIndia, Q2 KYC depth regex-only vs Razorpay fund-account API async-enrich, Q3 tier warehouse caps 1/5/∞ vs 3/10/∞, Q4 admin-approve-all vs auto-approve existing corporate-buyer-orgs, Q5 SLA scorer empty-arrays vs mock 95%). Tracked under Phase-6 Epic + Phase-Acceptance issues (created at execution time, not now).'
    status: in_progress
  - id: p7
