...
todos:
  ...
  - id: p6
    ...
    status: completed
  - id: p7
    content: "Phase 7 DONE via PR-17 (sub-plan .cursor/plans/p7_product-service_pr-17_a94f79e1.plan.md, squash-merged 2026-05-16 as 1a045daf). Scope: services/product-service end-to-end (5 Mongoose schemas + 8 services + 7 controllers + ProductOwnershipGuard + VendorActiveGuard + RoleGuard + 5 v1 outbox events) + Cloudflare R2 presigned PUT uploads (AWS SDK S3 v3 + 15min expiry + 5MB max + jpeg/png/webp) + Atlas Search snapshot sync via product.search_index denormalized collection (M0 $regex fallback in dev) + admin review moderation queue + corporate-gifting taxonomy (occasion/recipientType/categoryL1+L2/customizable/brandingAreas/moq/leadTimeDays/hsnCode) + variant subdoc-array (≤200, Map attributes). Populates P2 shells (@repo/validators/product 9 files, @repo/events/product 5 v1 events, @repo/types/product.ts). Introduces StockReadPort + StubStockReadPort at @repo/utils bound in apps/api-gateway (P8 swaps real impl). docs/architecture/cross-service-contracts.md formalizes cross-module port pattern. infrastructure/atlas/search/products.json for drift CI. microservice-boundaries + dep-cruiser relaxed for public-barrel re-use at composition root. 86 files / +7472 across 3 commits squashed (initial + Copilot 6-comment fix + vendorId ULID fix). 44 product-service tests across 8 spec files. Copilot: 6 inline comments addressed in c655a9b; vendorId≠orgId fixed in 2d42164. Closes Phase-7 Epic #39 + Phase-Acceptance #40; milestone #8 closed. Validated: 16/16 required CI checks green on merge SHA."
    status: completed
  - id: p8
    content: "Phase 8 DONE via PR-18 (sub-plan .cursor/plans/p8_inventory-service_pr-18_13ecfd53.plan.md, squash-merged 2026-05-16 as 96d4107). Scope: services/inventory-service end-to-end (5 Mongoose schemas + 11 services + 6 controllers + WarehouseOwnershipGuard + admin CLI transfer) + event-sourced stock ledger + materialized snapshots + Upstash Redis/in-memory TTL reservations + @Global InventoryServiceModule.forRoot binding RedisStockReadPort (replaces gateway StubStockReadPort) + ReservationPort at @repo/utils + 8 v1 outbox events + @repo/validators/inventory + @repo/types/inventory.ts + low-stock/dead-stock/reorder crons (@Cron Asia/Kolkata). docs/architecture/cross-service-contracts.md updated; free-tier-budget Upstash quota corrected (500K cmds/month). 79 files / +4047 across 2 commits squashed (feat + fix eslint/events coverage). 14 inventory-service tests across 8 spec files (+ utils/events port tests). CI: lint fail iter1 (eslint library import + events coverage) → fix iter2 all green. Closes Epic #42 + Phase-Acceptance #43; milestone #24 closed."
    status: completed
  - id: p8b
