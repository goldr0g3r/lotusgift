...
todos:
  ...
  - id: p4
    ...
    status: completed
  - id: p5
    content: 'Phase 5 SCAFFOLD DONE via PR-14 (squash-merged 2026-05-14 as 9686881). Ships services/auth-service options-builder + 3 decorators (@AllowAnonymous, @Session, @CurrentUser) + 2 DI tokens (AUTH_INSTANCE, AUTH_MONGO_CLIENT) + @repo/auth-client browser SDK. ⚠️ PARENT PLAN STATUS REMAINS in_progress because the FULL P5 (Better-Auth runtime + AuthGuard + toNodeHandler mount + Passkey/2FA/OTP/Google + KYC + cross-subdomain SSO Playwright) is DEFERRED to P5b — needs the user to choose the CJS↔ESM interop pattern (better-auth is ESM-only; api-gateway is CJS). The P4 /api/auth/* stub controller continues to return 503 until P5b lands. Closes Phase-5 Epic #31 + Phase-Acceptance #30 (scope was MVP-only). 2 commits squashed (initial + lockfile re-sync after dep trim). All 16 required CI checks green.'
    status: in_progress
  - id: p6
