...
todos:
  ...
  - id: p7
    ...
    status: completed
  - id: p8
    content: 'Phase 8 IN PROGRESS (planning complete, execution gated on P7 merge — now unblocked). Sub-plan .cursor/plans/p8_inventory-service_pr-18_13ecfd53.plan.md + research note docs/research/phase-8-inventory-service.md + Epic #42 + Phase-Acceptance #43 under milestone #24 (Phase 8 - Inventory Service). Scope: per-(variantId, warehouseId) event-sourced stock ledger + materialized snapshots + Upstash Redis TTL reservations + InventoryStockReadPort (replaces StubStockReadPort) + low-stock/dead-stock/reorder cron feeds + admin-CLI inter-warehouse transfer + WarehouseOwnershipGuard. PR-18 execution next.'
    status: in_progress
  - id: p8b
