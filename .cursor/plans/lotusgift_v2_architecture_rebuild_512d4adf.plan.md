...
todos:
  ...
  - id: p1
    ...
    status: completed
  - id: p2
    content: 'Phase 2: @repo/types + @repo/validators (Zod) + @repo/events (transport-agnostic event schemas + __schemaVersion) + @repo/openapi-spec (shared x-* extensions + RFC 9457 error envelope). IN PROGRESS via sub-plan .cursor/plans/p2_l1_leaf_packages_pr-10_8329fb87.plan.md (branch pr-10-l1-packages). Scope decisions: full polish in one PR (matches PR-9 pattern); foundation + 16-service skeleton folders per user foundation_plus_all decision. Adopt Zod 4 directly (lockfile already at 4.4.3 via transitive); nestjs-zod stays at L4 (api-gateway). RFC 9457 library choice deferred to P4. Tracked under Phase-2 Epic #18 + Phase-Acceptance #19 (created 2026-05-14 alongside this work).'
    status: in_progress
  - id: p3
