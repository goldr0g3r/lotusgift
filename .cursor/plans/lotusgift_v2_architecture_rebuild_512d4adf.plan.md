...
todos:
  ...
  - id: p2
    ...
    status: completed
  - id: p3
    content: 'Phase 3: @repo/database (Mongoose + collection-namespace helper) + @repo/config (env Zod schema) + @repo/utils (OutboxPort + redactor + ulid trace-id + pino logger + retry) + @repo/observability (OTEL bootstrap + RUM SDK init). IN PROGRESS via sub-plan .cursor/plans/p3_l2_platform_packages_pr-11_6726a624.plan.md (branch pr-11-l2-packages). Scope decisions: full polish in one PR (matches PR-9/PR-10); observability=OTEL-only per user recommend (Sentry deferred to P21, PostHog to P3b); OutboxPort full MVP at L2 (interface + MongoOutboxRepository + InProcessOutboxPort relayer + LRU idempotency dedup), Nest binding at L4 (P4). Tracked under Phase-3 Epic #21 + Phase-Acceptance #22 (created 2026-05-14 alongside this work).'
    status: in_progress
  - id: p3b
