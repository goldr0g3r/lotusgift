...
todos:
  ...
  - id: p4
    ...
    status: completed
  - id: p5
    content: 'Phase 5 SCAFFOLD: services/auth-service options-builder + decorators + DI tokens + @repo/auth-client browser SDK. SCAFFOLD shipped via PR-14 (autonomous; squash-merge pending user review). DEFERRED to P5b (real merge needed when user is back): construct + register betterAuth() instance inside an async Nest provider with the right CJS↔ESM interop (better-auth is ESM-only; api-gateway is CJS); AuthGuard + toNodeHandler mount at /api/auth/* replacing P4 stub; Passkey/WebAuthn; 2FA TOTP + backup codes; Phone OTP via MSG91; Google social; email verification + password reset (needs P12 email infra); KYC + credit-limit underwriting (P6 + P10); cross-subdomain SSO Playwright test (P16); warehouse-manager + inventory-manager team memberships (P6). Decorators (@AllowAnonymous, @Session, @CurrentUser) + tokens (AUTH_INSTANCE, AUTH_MONGO_CLIENT) + buildBetterAuthOptions(env) are ready for consumer use TODAY. Tracked under Phase-5 Epic #31 + Phase-Acceptance #30 (numbers swapped because the first Epic create failed on a bad area/auth label).'
    status: in_progress
  - id: p6
