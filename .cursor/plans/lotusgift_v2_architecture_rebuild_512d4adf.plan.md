...
todos:
  ...
  - id: p0-dev-stack
    ...
    status: completed
  - id: p0-design
    content: "PR-6 docs(design)+feat(design-tokens): docs/design/DESIGN.md (LotusGift palette + voice) + @repo/design-tokens (port brand.green/pink/ink + Plus Jakarta + Geist + radius incl. 4xl/5xl/organic-blob + shadows panel/pill/elevated/glow + 13 animation keyframes from current apps/web/tailwind.config.ts) + @repo/ui baseline on Radix Primitives + CSS Modules + Sass (Button + IconButton + Card + Pill + SectionShell + Toaster + Sass mixins for btn-primary / btn-pink / btn-outline / btn-disc / eyebrow / h1-display / icon-circle). WCAG 2.2 AA floor + @axe-core/playwright in CI."
    status: pending
  - id: p0-oracle-runbook
    content: "PR-7 docs(runbook)+infra(oracle): docs/runbooks/oracle-deploy.md + infrastructure/oracle/ (docker-compose.prod.yml + nginx + Certbot + UFW + fail2ban + systemd + heartbeat-ping cron every 6h mitigating 7-day idle-reclaim + GHCR pull credentials) + .github/workflows/deploy-oracle.yml (build → push GHCR → SSH zero-downtime swap)."
    status: pending
  - id: p0-future-docs
    content: "PR-8 docs(runbook): going-to-production.md + scaling-up.md (microservice-split triggers, Atlas M0→M10 migration, PostHog self-host for India DPDP, mobile apps, marketplace-operated fulfilment centres FBA-style post-launch) + free-tier-burn.md + incident-response.md + backup-restore.md + oracle-quarterly-review.md."
    status: pending
  ...
  - id: p1
    ...
    status: pending
  - id: p2
    content: "Phase 2: @repo/types + @repo/validators (Zod) + @repo/events (transport-agnostic event schemas + __schemaVersion) + @repo/openapi-spec (shared x-* extensions + RFC 9457 error envelope)."
    status: pending
  - id: p3
    content: "Phase 3: @repo/database (Mongoose + collection-namespace helper) + @repo/config (env Zod schema) + @repo/utils (OutboxPort + redactor + ulid trace-id + pino logger + retry) + @repo/observability (OTEL bootstrap + RUM SDK init)."
    status: pending
  - id: p3b
    content: "Phase 3b: @repo/analytics-sdk (PostHog browser+server) + @repo/feature-flags (PostHog flags) + event taxonomy doc (corporate-gifting events: rfq.opened, customization.mockup.approved, recipient-list.uploaded, order.routed-to-rfq, etc.)."
    status: pending
  - id: p4
    content: "Phase 4: apps/api-gateway modular-monolith shell — trace-id middleware + Upstash Redis rate-limit + helmet + CSP + Better-Auth mount point at /api/auth/* (preserves current apps/api/src/main.ts pattern: bodyParser disabled, toNodeHandler before NestJS, raw-body capture for /api/payments/webhook) + RFC 9457 error envelope + @thallesp/nestjs-better-auth AuthGuard + @nestjs/swagger spec endpoint + Kubb codegen wired in CI → @repo/api."
    status: pending
  - id: p5
    content: "Phase 5: services/auth-service module + @repo/auth-client — Better-Auth + @better-auth/mongo-adapter + email/password + email verification + password reset + multi-session + Passkey/WebAuthn (mandatory admin; optional vendor/corporate-buyer) + 2FA TOTP + backup codes (mandatory admin/vendor; optional corporate-buyer/individual) + Phone OTP via MSG91 + Google social + Organization plugin (3 org types: vendor-org with self-serve+admin-approval-gate, corporate-buyer-org NEW with KYC+PO-terms+credit-limit+approval-matrix, internal-staff-org with custom roles+teams; warehouse-manager+inventory-manager memberships scoped via vendor-org teams — one team per warehouse, team-id == warehouseId) + Admin plugin (impersonation / ban / force-sign-out). Single-individual retail buyers still allowed (no org). Test cross-subdomain SSO for 4 Next apps via cookie domain."
    status: pending
  - id: p6
    content: "Phase 6: services/vendor-service module — self-serve onboarding wizard + KYC (GSTIN / PAN / bank) + admin approval gate before activation (vendor cannot list products until activated) + multi-warehouse registry (geocoded address + state + city + pincode + lat/lng + operating hours + pickup SLA + carrier cut-off times + service-zone polygon-or-pincode-list + enable/disable + ownerType: vendor|platform forward-compat) + vendor subscription tiers + sliding commission schedule + payouts ledger + per-warehouse SLA scoring."
    status: pending
  - id: p7
    content: "Phase 7: services/product-service module — corporate-gifting taxonomy (occasion / recipientType / customizable / brandingAreas / moq / leadTimeDays / sampleAvailable / hsnCode) + Cloudflare R2 image upload + outbox-publishes product events + MongoDB Atlas Search index sync for products (facets on occasion/recipientType/categoryId/vendorId, autocomplete, fuzzy). Products vendor-scoped; per-warehouse stock joined at read time from inventory-service."
    status: pending
  - id: p8
