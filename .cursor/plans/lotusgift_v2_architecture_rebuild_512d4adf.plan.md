...
todos:
  ...
  - id: p4
    ...
    status: completed
  - id: p5
    content: 'Phase 5 SCAFFOLD DONE via PR-14 (squash-merged 2026-05-14 as 9686881). Ships services/auth-service options-builder + 3 decorators (@AllowAnonymous, @Session, @CurrentUser) + 2 DI tokens (AUTH_INSTANCE, AUTH_MONGO_CLIENT) + @repo/auth-client browser SDK. Phase 5b RUNTIME WORK IN PROGRESS via PR-15 (sub-plan .cursor/plans/p5b_auth_service_runtime_pr-15_4920fca6.plan.md, branch pr-15-auth-runtime) — wires the real betterAuth(...) instance via async dynamic-import providers (CJS↔ESM interop via await import() inside useFactory; no "type": "module" cascade), recreates AuthServiceModule + AuthGuard, replaces the /api/auth/* stub with toNodeHandler(auth) mount, turns on Passkey/WebAuthn (@better-auth/passkey) + 2FA TOTP + Phone OTP via MSG91 + Google social + email-verify/password-reset stubs. Parent plan p5 flips to completed at PR-15 merge.'
    status: in_progress
  - id: p6
