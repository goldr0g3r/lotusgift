...
todos:
  ...
  - id: p0-issues
    ...
    status: completed
  - id: p1
    content: 'Phase 1: @repo/typescript-config + @repo/eslint-config + @repo/jest-config + @repo/prettier-config polish. IN PROGRESS via sub-plan .cursor/plans/p1_shared_configs_pr-9_c3e29291.plan.md (branch pr-9-shared-configs). Scope decisions: full polish in one PR (matches PR-6/7/8 pattern); TS 6 modernize (drop Node10 + baseUrl) per user Q2-decision, cascade .js extensions into apps/api-gateway/src. Tracked under Phase-1 Epic #15 + Phase-Acceptance #16 (created 2026-05-14 alongside this work).'
    status: in_progress
  - id: p2
