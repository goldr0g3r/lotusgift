...
todos:
  ...
  - id: p0-oracle-runbook
    ...
    status: completed
  - id: p0-future-docs
    content: "PR-8 docs(runbook): going-to-production.md + scaling-up.md (microservice-split triggers, Atlas M0→M10 migration, PostHog self-host for India DPDP, mobile apps, marketplace-operated fulfilment centres FBA-style post-launch) + free-tier-burn.md + incident-response.md + backup-restore.md + oracle-quarterly-review.md + local-development.md (NEW, host install preferred over docker compose per user preference). IN PROGRESS via sub-plan .cursor/plans/p0-future-docs_pr-8_6c3aa171.plan.md (branch pr-8-future-docs). Scope: all 7 runbooks at medium depth (github-setup.md template), single PR, docs-only (no CI surface changes). Tracked under Phase-0 Epic #4."
    status: in_progress
