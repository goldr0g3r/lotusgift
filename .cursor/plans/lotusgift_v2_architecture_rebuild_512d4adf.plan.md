...
todos:
  ...
  - id: p3b
    ...
    status: completed
  - id: p4
    content: 'Phase 4: apps/api-gateway modular-monolith shell. IN PROGRESS via branch pr-13-api-gateway. Scope decisions: shell-only (Better-Auth + rate-limit are stubs gated for P5); main.ts production bootstrap (bodyParser:false + rawBody + OTEL bootstrap-before-nest + Helmet with CSP relaxed on /api/docs* + structured CORS + Pino logger + cleanupOpenApiDoc + Swagger UI at /api/docs + JSON at /api/docs-json + graceful SIGTERM/SIGINT); app.module.ts wires ConfigModule (loadEnv from @repo/config) + MongooseModule (default-connection from MONGODB_URI) + nestjs-pino LoggerModule + GlobalProblemDetailsFilter (RFC 9457 from @repo/openapi-spec, handles ZodError + HttpException + generic Error) + global ZodValidationPipe + global ZodSerializerInterceptor + TraceIdMiddleware (AsyncLocalStorage scope echoed via X-Trace-Id header) + OUTBOX_PORT provider with OnApplicationBootstrap (start) + OnApplicationShutdown (await stop) + AuthMountStubController (503 until P5). /readyz endpoint probes Mongo readyState. kubb.config.ts + scripts/codegen/export-openapi.ts placeholder pipeline + pnpm api:generate script. Tracked under Phase-4 Epic #27 + Phase-Acceptance #28.'
    status: in_progress
  - id: p5
